...
// OnUnregisterClient called on agent disconnection so send a HostGraphDeleted message to all the peers
// so that they will delete the sub graph as well.
func (a *TopologyForwarder) OnUnregisterClient(c *shttp.WSClient) {
	for _, peer := range a.peers {
		if peer.wsclient != nil {
			msg := shttp.NewWSMessage(graph.Namespace, "HostGraphDeleted", c.Host)
