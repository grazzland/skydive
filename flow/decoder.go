...
func decodeInGRELayer(data []byte, p gopacket.PacketBuilder) error {
	ipPrefix, err := ipDecoderFromRawData(data, p)
	if ipPrefix {
		return err
	} else {
...
var LayerTypeInMplsEthOrIp = gopacket.RegisterLayerType(55556, gopacket.LayerTypeMetadata{
	Name:    "LayerTypeInMplsEthOrIp",
	Decoder: gopacket.DecodeFunc(decodeInMplsEthOrIpLayer)})
