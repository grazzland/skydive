...
func (f *TableClient) LookupFlowsByNodes(hnmap topology.HostNodeTIDMap, flowSearchQuery FlowSearchQuery) (*FlowSet, error) {
	...
	for host, tids := range hnmap {
		// copy to avoid Filter override betsween two calls on two different host
		query := flowSearchQuery

		query.Filter = NewAndFilter(NewFilterForNodeTIDs(tids), flowSearchQuery.Filter)
		go f.lookupFlows(ch, host, query)
	}
