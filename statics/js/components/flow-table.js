...
Vue.component('flow-table-control', {
  ...
  data: function() {
    return {
      query: "G.Flows().Limit(500).Sort()",
      validatedQuery: "G.Flows().Limit(500).Sort()",
      validationId: null,
      error: "",
    };
  },
  ...
  watch: {
    ...
    query: function(newQuery) {
      ...
      if (this.validationId) {
        clearTimeout(this.validationId);
        this.validationId = null;
      }
      this.validationId = setTimeout(function() {
        TopologyAPI.query(newQuery)
          .then(function() {
            self.validatedQuery = newQuery;
            self.error = "";
          })
          .fail(function(e) {
